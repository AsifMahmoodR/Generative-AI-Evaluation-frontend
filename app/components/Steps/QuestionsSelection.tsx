...
const QuestionsSelection = () => {
  ...
  var questions = evalationStore.questions;
  useEffect(() => {
      ...
      <div className="consent-container">
        <div className="lg:ml-6 lg:col-start-2 lg:max-w-5xl">
          ...
          <p className="mt-10 text-xl tracking-tight text-gray-900">
            Please select a question to view relevant responses:</p>
        </div>

        <div className="grid grid-cols-1 gap-4 px-1 py-5">
          {questions.map((c) => (

            <div onClick={() => onSelected(c.id)}

              className={isSelected(c.id)? 'question-item selected' : 'question-item'}
              key={c.id}>{c.question}</div>
          ))}
        </div>
      </div>
