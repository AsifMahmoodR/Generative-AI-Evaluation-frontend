        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:px-5 py-5 content-evenly">
          {categories.map((c) => (
            <div onClick={() => onSelected(c.id)}
              ...
              className={`category-item topic-name max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:text-white dark:border-gray-700 ${isSelected(c.id) ? 'selected' : ''}`}
